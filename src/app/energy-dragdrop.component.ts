...
export class EnergyDragdropComponent {
  ...
  showAddDialog = false;
  customApplianceName = '';
  customApplianceWatts = 0;
  customApplianceMinutes = 60;

  appliances: {
    [key: string]: { label: string; watt: number; image: string };
  } = {
    chuveiro: {
      label: 'Chuveiro Elétrico',
      watt: 7000,
      image: 'assets/appliances/chuveiro.png',
    },
    geladeira: {
      label: 'Geladeira',
      watt: 150,
      image: 'assets/appliances/geladeira.png',
    },
    tv: {
      label: 'TV',
      watt: 140,
    ...
    microondas: {
      label: 'Micro-ondas',
      watt: 1000,
      image: 'assets/appliances/microondas.png',
    },
    ar: {
      label: 'Ar-condicionado',
      watt: 1200,
      image: 'assets/appliances/ar.png',
    },
  };
  ...
  barChartOptions: any = {
    ...
    scales: {
      x: {
        ticks: { color: '#ffffff', font: { size: 16, weight: 'bold' } },
        grid: { color: '#444' },
      },
      y: {
        ticks: { color: '#ffffff', font: { size: 16, weight: 'bold' } },
  ...
  openAddDialog() {
    ...
    this.customApplianceName = '';
    this.customApplianceWatts = 0;
    this.customApplianceMinutes = 60;
  }
  ...
  addCustomAppliance() {
    if (
      !this.customApplianceName.trim() ||
      this.customApplianceWatts <= 0 ||
      this.customApplianceMinutes <= 0
    ) {
      alert('Por favor, preencha todos os campos corretamente.');
      return;
    }

    // Generate a unique key for the custom appliance
    const customKey = 'custom_' + Date.now();

    // Add to appliances dictionary with a generic image
    this.appliances[customKey] = {
      label: this.customApplianceName.trim(),
      watt: this.customApplianceWatts,
      image: 'https://cdn-icons-png.flaticon.com/512/16/16686.png', // Using the generic test image
    };

    // Add to selected appliances
    this.selectedAppliances.push(customKey);

    // Set usage with custom minutes
    this.usage[customKey] = {
  ...
  generateSavingsTips() {
    ...
    const topSpenderKey =
      this.ranking.length > 0
        ? this.selectedAppliances.find(
            (k) => this.appliances[k].label === this.ranking[0].nome
          )
        : undefined;
    if (topSpenderKey) {
      ...
      if (
        applianceUsage.minutesPerDay > 60 &&
        applianceInfo.label !== 'Geladeira'
      ) {
        ...
        if (monthlySavingBRL > 1) {
          this.savingsTips.push(
            `Seu maior vilão é '${
              applianceInfo.label
            }'. Reduzir seu uso em 30 minutos por dia pode economizar cerca de R$ ${monthlySavingBRL.toFixed(
              2
            )} por mês.`
          );
        }
    ...
    this.selectedAppliances.forEach((key) => {
      ...
      const monthlyCost =
        (applianceUsage.powerW / 1000) *
        (applianceUsage.minutesPerDay / 60) *
        applianceUsage.quantity *
        30 *
        this.globalTariff;
      ...
      switch (key) {
        case 'chuveiro':
          ...
          if (applianceUsage.minutesPerDay > reductionMinutes) {
            const dailySavingKwh =
              (applianceUsage.powerW / 1000) *
              (reductionMinutes / 60) *
              applianceUsage.quantity;
            const monthlySavingBRL = dailySavingKwh * 30 * this.globalTariff;
            this.savingsTips.push(
              `O chuveiro elétrico é um grande consumidor. Reduzir ${reductionMinutes} minutos do banho diário pode gerar uma economia de R$ ${monthlySavingBRL.toFixed(
                2
              )} por mês.`
            );
          }
        ...
        case 'ar':
          if (applianceUsage.minutesPerDay > 120) {
            ...
            this.savingsTips.push(
              `Para o ar-condicionado, aumentar a temperatura em 1°C pode economizar ~7% de energia. Isso representaria R$ ${monthlySavingBRL.toFixed(
                2
        ...
        case 'geladeira':
          ...
          if (applianceUsage.powerW > modernFridgeWatts + 50) {
            ...
            const monthlySavingBRL =
              (currentMonthlyKwh - modernMonthlyKwh) * this.globalTariff;
            if (monthlySavingBRL > 10) {
              this.savingsTips.push(
                `Sua geladeira consome bastante. Avalie a troca por um modelo A+++, o que pode economizar até R$ ${monthlySavingBRL.toFixed(
                  2
                )} por mês.`
              );
            }
        ...
        case 'tv':
          ...
          if (standbyHours > 8) {
            ...
            if (monthlySavingBRL > 1.5) {
              this.savingsTips.push(
                `Desligar a(s) TV(s) da tomada pode economizar cerca de R$ ${monthlySavingBRL.toFixed(
                  2
                )} por mês em "energia fantasma".`
              );
            }
          }
          break;
        ...
        case 'microondas':
          ...
          if (monthlySavingBRL > 1) {
            this.savingsTips.push(
              `O relógio do micro-ondas também gasta. Desligá-lo da tomada pode economizar R$ ${monthlySavingBRL.toFixed(
                2
              )} por mês.`
            );
          }
